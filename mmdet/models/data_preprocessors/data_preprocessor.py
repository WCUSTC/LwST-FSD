...
@MODELS.register_module()
class DetDataPreprocessorMultiImages(DetDataPreprocessor):
    '''专门针对多帧输入
    输入的data["input"]为list，每个元素代表一个样本，每个样本的形状为[T,3,H,W]
    '''
    ...
    def stack_batch_my(self,tensor_list: List[torch.Tensor],
                    pad_size_divisor: int = 1,
        ...
        pad[:, 1::2] = padded_sizes[:, range(dim - 1, -1, -1)] ##左边/上边不padding
        batch_tensor = []
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        batch_pad_shape = self._get_pad_shape(data)
        data =self.superforward(data=data, training=training)
        inputs, data_samples = data['inputs'], data['data_samples']
    ...
    def superforward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for _batch_input in _batch_inputs:
                ...
                # Normalization.
                if self._enable_normalize:#'''这里暂时没用到，没改。'''
            ...
            batch_inputs = self.stack_batch_my(batch_inputs, self.pad_size_divisor, self.pad_value)
        # Process data with `default_collate`.
        elif isinstance(_batch_inputs, torch.Tensor):
            assert _batch_inputs.dim() == 5, (
                'The input of `ImgDataPreprocessor` should be a NTCHW tensor '
                'or a list of tensor, but got a tensor with shape: '
                f'{_batch_inputs.shape}')
            if self._channel_conversion:
            ...
            pad_h = target_h - h
            pad_w = target_w - w
            batch_inputs = F.pad(_batch_inputs, (0,0,0, pad_w, 0, pad_h),
...
@MODELS.register_module()
class BatchSyncRandomResizeMultiImages(BatchSyncRandomResize):
