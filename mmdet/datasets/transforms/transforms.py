...
@TRANSFORMS.register_module()
class ResizeMultiImages(Resize):
            ...
            if self.keep_ratio:
                ...
                for i in range(100):
                    ...
                    if key_name not in results.keys():
                        ...
                        results[key_name],_ = mmcv.imrescale(
                            results[key_name],
                            results['scale'],
                            interpolation=self.interpolation,
                            return_scale=True,
                ...
                for i in range(100):
                    ...
                    if key_name not in results.keys():
                        ...
                        results[key_name],_,_ =  mmcv.imresize(
                            results[key_name],
                            results['scale'],
                            interpolation=self.interpolation,
                            return_scale=True,
...
@TRANSFORMS.register_module()
class RandomFlipMultiImages(RandomFlip):
    @autocast_box_type()
    def _flip(self, results: dict) -> None:
        """Flip images, bounding boxes, and semantic segmentation map."""
        # flip image
        ...
        for i in range(100):
            ...
            if key_name not in results.keys():
                ...
                results[key_name] =mmcv.imflip(results[key_name], direction=results['flip_direction'])
...
@TRANSFORMS.register_module()
class MosaicMultiImages(Mosaic):
    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        """Mosaic transform function.
        ...
        for i in range(100):
            ...
            if key_name not in results.keys():
                ...
                img_lst = [results[key_name]]
                for res in results['mix_results']:
                    img_lst.append(res[key_name])
                results[key_name] = self.single_img(center_position=center_position,img_lst=img_lst)
        ## 目标帧图片、标注一起操作
        loc_strs = ('top_left', 'top_right', 'bottom_left', 'bottom_right')
        for i, loc in enumerate(loc_strs):
            if loc == 'top_left':
    ...
    def single_img(self,center_position,img_lst):
        if len(img_lst[0].shape) == 3:
            mosaic_img = np.full(
                (int(self.img_scale[1] * 2), int(self.img_scale[0] * 2), 3),
                self.pad_val,
                dtype=img_lst[0].dtype)
        else:
            mosaic_img = np.full(
                (int(self.img_scale[1] * 2), int(self.img_scale[0] * 2)),
                self.pad_val,
...
class MixUpMultiImages(MixUp):
    ...
    def transform(self, results: dict) -> dict:
        ...
        scale_ratio = min(self.dynamic_scale[1] / retrieve_img.shape[0],
                          self.dynamic_scale[0] / retrieve_img.shape[1])
        scale_ratio_org = copy.deepcopy(scale_ratio)
        retrieve_img = mmcv.imresize(
            retrieve_img, (int(retrieve_img.shape[1] * scale_ratio),
                           int(retrieve_img.shape[0] * scale_ratio)))
        ...
        for i in range(100):
            ...
            if key_name not in results.keys():
                ...
                results[key_name] = self.single_img(
                    retrieve_img=results['mix_results'][0][key_name],
                    ori_img = results[key_name],
                    scale_ratio=scale_ratio_org, jit_factor=jit_factor, is_flip=is_flip,x_offset=x_offset, y_offset=y_offset)
        return results

    def single_img(self,retrieve_img,ori_img,scale_ratio,jit_factor,is_flip,x_offset, y_offset):
...
@TRANSFORMS.register_module()
class RandomAffineMultiImages(RandomAffine):
    @autocast_box_type()
    def transform(self, results: dict) -> dict:
        img = results['img']
        height = img.shape[0] + self.border[1] * 2
        ...
        for i in range(100):
            ...
            if key_name not in results.keys():
                ...
                results[key_name] = cv2.warpPerspective(
                    results[key_name],
                    warp_matrix,
                    dsize=(width, height),
                    borderValue=self.border_val)
...
@TRANSFORMS.register_module()
class YOLOXHSVRandomAugMultiImages(YOLOXHSVRandomAug):
    def transform_single(self,img,hsv_gains):
        img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.int16)
        img_hsv[..., 0] = (img_hsv[..., 0] + hsv_gains[0]) % 180
        img_hsv[..., 1] = np.clip(img_hsv[..., 1] + hsv_gains[1], 0, 255)
        img_hsv[..., 2] = np.clip(img_hsv[..., 2] + hsv_gains[2], 0, 255)
    ...
    def transform(self, results: dict) -> dict:
        hsv_gains = self._get_hsv_gains()
        img = self.transform_single(results['img'],hsv_gains)
        results['img'] = img
        for i in range(100):
            key_name = "img_%04d"%i
            if key_name not in results.keys():
                ...
                results[key_name] =self.transform_single(results[key_name],hsv_gains)
        return results
