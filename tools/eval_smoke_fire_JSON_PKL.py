...
def parse_args():
    ...
    parser.add_argument('path_res_pkl', help='results path')
    parser.add_argument('path_ann_json', help='json file path')
...
class smoke_fire_evaluator:
    def __init__(self,
        ...
        with open(path_ann_json,'r') as f:
            self.annotations_org = json.load(f)
        self.annotations_all=[]
        for ann in self.annotations_org:
            if len(ann["bboxes"])==0:
                ann["bboxes"] = np.zeros((0,4), dtype=np.float32)
                ann["labels"] = np.zeros((0,), dtype=np.int64)
            else:
                ann["bboxes"]=np.asarray(ann["bboxes"],dtype=np.float32)
                ann["labels"]=np.asarray(ann["labels"],dtype=np.int64)              +1  ###标签json中smoke是0，代码中是1
            ann["bboxes_ignore"] = np.zeros((0, 4))
            ann["labels_ignore"] = np.zeros((0, ))
            self.annotations_all.append(copy.deepcopy(ann))
        self.img_lst_fromann = [v["img_path"] for v in self.annotations_all ]
        with open(path_res_pkl, 'rb') as f:
            self.results_org = pickle.load(f)
            result_dict = {v["img_path"]:v for v in self.results_org}

        self.results_all =[]
        for img_path in self.img_lst_fromann:
            if img_path not in result_dict.keys():##无预测结果的图片要补空
                print(img_path,"not in results")
                self.results_all.append([np.zeros((0,5),dtype=np.float32), np.zeros((0,5),dtype=np.float32)])
                continue
            res = result_dict[img_path]#以annotation为基准找到results_org
            res = res["pred_instances"]
            pred = [[], []]
            for label, score, bbox in zip(res['labels'].data.cpu().numpy().tolist(),
                                          res['scores'].data.cpu().numpy().tolist(),
                                          res['bboxes'].data.cpu().numpy().tolist()):
    ...
    def evaluate(self,logger=None):
        ...
        results = self.results_all
        annotations = self.annotations_all
...
if __name__=="__main__":
    ...
    args = parse_args()
    evaluator = smoke_fire_evaluator(path_res_pkl=args.path_res_pkl,path_ann_json=args.path_ann_json,
                                     iou_thr=args.iou_thr,score_thr=args.score_thr,metrics=args.metrics)
